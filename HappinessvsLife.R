...
selected_data <- csv1[, c("Country", "Happiness.Score", "Health..Life.Expectancy.")]
place <- csv1[, c("Country")]
place1 <- place[1:50]
Death <- csv1[,c("Happiness.Score")]
Death1 <- Death[1:50]
Death1 <- Death1/max(Death1)*100
happy <- csv1[,c("Health..Life.Expectancy.")]
happy1=happy[1:50]
...
png("~/BDA/2015HvL.png", width = 800, height = 600)
barplot(df, main = "Happiness.Score", names.arg =place1,  
        xlab = "Country", ylab = "Health..Life.Expectancy.",  
